...
def getvals():
 ...
 print(f"{namevalue.get(),gendervalue.get(),contactvalue.get(),paymentmodevalue.get(),cityvalue.get(),FoodServicevalue.get()}")

 with open("records.text","a") as f:
    f.write(f"{namevalue.get(),gendervalue.get(),contactvalue.get(),paymentmodevalue.get(),cityvalue.get(),FoodServicevalue.get()}\n")
...
Button(text ="submit to srija travels",font=30,bg="black",fg="red",command=getvals).grid(row=7,column=3)
